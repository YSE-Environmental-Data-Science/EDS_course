...
```{r, include=T}
Yale.Myers$date <- paste( Yale.Myers$year, Yale.Myers$yday, sep="-") %>% as.Date(format="%Y-%j")
```

Look at your work:

```{r, include=T}
head(Yale.Myers$year.day)
```
...
```{r, include=T}
Yale.Myers.annual <-Yale.Myers.sub %>%reframe(.by=year, tmax = max(tmax), tmin = min(tmin), tmean = mean(tmean), prcp = sum(prcp))
```
...
```{r, include=T}
Yale.Myers.monthly <-Yale.Myers.sub %>% reframe(.by =month, tmax = max(tmax), tmin = min(tmin), tmean = mean(tmean), prcp = sum(prcp)/29)
```
...
```{r, include=T}
Yale.Myers.yearmon <-Yale.Myers.sub %>% reframe(.by= c(month, year) , tmax = max(tmax), tmin = min(tmin), tmean = mean(tmean), prcp = sum(prcp))
```
...
```{r, include=T, eval=F}
save(Yale.Myers, Yale.Myers.sub,Yale.Myers.annual.tmean, file="data/products/DataBasics.RDATA" )
```
