...
Step 1. Use the year and day of the year to create a year_day variable.

```{r}
yale_myers$year_day <- paste(yale_myers$year, yale_myers$yday, sep = "-") %>% as.Date(format = "%Y-%j")
```
...
Step 2. Format the year_day as YYYY-mm-dd and convert to a date.

```{r}
yale_myers$date <- format(yale_myers$year_day, "%Y-%m-%d") %>% as.Date()
```
...
The function `mutate()` creates new columns that are a function of existing variables. Use `mutate()` to create a month (month) and Julian day (julian_day) from the formatted date.

```{r}
yale_myers <- yale_myers %>% mutate(month = format(yale_myers$date, format = "%m"),
                                     julian_day = format(yale_myers$date, format = "%j"))
```

With daymet data you have to calculate the mean temperature using tmax and tmin:

```{r}
yale_myers$tmean <- (0.606 * as.numeric(yale_myers$tmax..deg.c.)) + 0.394 * as.numeric(yale_myers$tmin..deg.c.)
```

Look at your work:

```{r}
summary(yale_myers$tmean)
```
...
```{r}
yale_myers_rn <- yale_myers %>% rename(tmax = tmax..deg.c., tmin = tmin..deg.c., prcp = prcp..mm.day.)
```
...
Subset the dataset to include only \[date, month, year, julian_day, tmax, tmin, tmean, and prcp\] :

```{r}
yale_myers_sub <- yale_myers_rn %>% select(date, month, year, julian_day, tmax, tmin, tmean, prcp)
```
...
```{r}
yale_myers_annual <- yale_myers_sub %>% reframe(.by = year, tmax = max(tmax), tmin = min(tmin), tmean = mean(tmean), prcp = sum(prcp))
```
...
Determine the average monthly conditions using `reframe()`:

```{r}
yale_myers_monthly <- yale_myers_sub %>% reframe(.by = month, tmax = max(tmax), tmin = min(tmin), tmean = mean(tmean), prcp = sum(prcp) / 29)
```
...
```{r}
yale_myers_yearmon <- yale_myers_sub %>% reframe(.by = c(month, year), tmax = max(tmax), tmin = min(tmin), tmean = mean(tmean), prcp = sum(prcp))
```
...
```{r}
yale_myers_annual_tmean <- yale_myers_annual %>% mutate(yale_myers = tmean) %>% select(year, yale_myers)
```
...
```{r}
#| eval: false
save(yale_myers, yale_myers_sub, yale_myers_annual_tmean, file = "data/products/data_basics.RDATA")
```
...
```{r}
ggplot(data = yale_myers_sub, aes(x = date, y = tmean)) +
  labs(title = "Climate of Yale Myers Forest")
```
...
```{r}
# Adding size
ggplot(data = yale_myers_sub, aes(x = date, y = tmean, size = year)) +
  geom_point() +
  labs(title = "Climate of Yale Myers Forest",
       x = "Date",
       y = "Mean Air Temperature (Degrees Celcius)")
...
ggplot(data = yale_myers_sub, aes(x = date, y = tmean, col = month)) +
  geom_point() +
  labs(title = "Climate of Yale Myers Forest",
       x = "Date",
       y = "Mean Air Temperature (Degrees Celcius)")
...
# Histogram plot
ggplot(data = yale_myers_sub, aes(x = tmean)) +
...
```{r}
# Subset dataset to include only months 9-11
yale_myers_fall <- yale_myers_sub %>% filter(month > "08" & month < "12")

p <- ggplot(data = yale_myers_fall, aes(x = date, y = tmean)) + geom_point()
...
```{r}
# Subset the data:
yale_myers_feb <- yale_myers_sub %>% filter(month == "02")

ggplot(data = yale_myers_feb, aes(x = date, y = tmean)) + geom_point() +
...
```{r}
ggplot(data = yale_myers_fall, aes(x = tmin, y = tmean)) + geom_point() +
  stat_smooth(method = lm, col = "red") +
...
-   Create a report to upload to canvas (save as a pdf). Consider creating this report in Rmarkdown: <https://hbctraining.github.io/Training-modules/Rmarkdown/lesson.html>
