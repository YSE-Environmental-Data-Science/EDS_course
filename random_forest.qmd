...
Read in the data. The chunk below loads it silently so the rest of this document has the data available; the next chunk shows the same command for you to run in your own session (it's set not to re-run here since the data is already loaded):
...
Our ultimate interest is in predicting monthly methane fluxes using both dynamic and static attributes of ecosystems. Before we start modeling with the data, it is a good practice to first visualize the variables. The `ggpairs()` function from the GGally package is a useful tool that visualizes the distribution and correlation between variables in a single grid of plots:
...
```{r}
set.seed(111) # set the random number generator
...
1. keep.forest = TRUE: This will save the random forest output, which will be helpful in summarizing the results.
...
```{r}
fch4_rf <- randomForest(FCH4_F_gC ~ P_F + TA_F + VPD_F,
                         data = train,
                         keep.forest = TRUE,
                         importance = TRUE,
                         mtry = 1,
...
```{r}
train$pred_fch4 <- predict(fch4_rf, train)
head(train$pred_fch4)
```

Compare the observed (FCH4_F_gC) versus predicted (pred_fch4):

```{r}
ggplot() + geom_point(data = train, aes(x = FCH4_F_gC, y = pred_fch4)) +
  geom_smooth(method = "lm")

summary(lm(data = train, pred_fch4 ~ FCH4_F_gC))
```

See how well the model performs on data that was not used to train the model:

```{r}
test$pred_fch4 <- predict(fch4_rf, test)

ggplot() + geom_point(data = test, aes(x = FCH4_F_gC, y = pred_fch4)) +
  geom_smooth(method = "lm")

summary(lm(data = test, pred_fch4 ~ FCH4_F_gC))
```
...
```{r}
#| eval: false
save(fch4_rf, file = "data/products/random_forest_model.RDATA")
```
